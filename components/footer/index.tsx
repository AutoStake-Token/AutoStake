...
function Footer() {
  return (
    <div tw="flex flex-col items-center  h-full justify-center relative ">
      <div tw="h-1 width[95%] background-color[var(--green)] my-8"></div>
      <div tw="flex flex-col-reverse md:flex-row md:items-center justify-center py-8 space-y-12 md:space-y-0 md:space-x-24 px-8 md:px-24 h-full w-full ">
        <div tw=" flex flex-1 flex-col space-y-12 items-start pt-8">
          <img
            ...
            alt=""
          />
          <div tw="flex flex-col md:flex-row color[var(--greenDark)] text-lg">
            <p tw="flex-1 text-center mb-8">
            ...
            <p tw="flex-1 text-center">
              You should be aware of all the risks associated with
              cryptocurrency trading. Please trade wisely!
              <br /> -ReBased Team
            </p>
          </div>
        </div>
        ...
        <div tw="flex flex-col lg:flex-row flex-1 ">
          <div tw="flex flex-col space-y-4  mr-16 mb-8">
            <span
              tw="text-7xl color[var(--green)] mb-4 md:mb-12  "
              className="text-glow"
            >
              support
            </span>
              ...
              {footerdataLinks.map((data, index) => (
                <a href={data.link} key={index}>
                  <div tw="flex">
                    ...
                    <span tw="color[var(--green)] capitalize cursor-pointer font-bold hover:color[var(--greenDark)]">
                      {data.title}
                    </span>
                  </div>
                </a>
          ...
          <div tw="flex flex-col space-y-4  ">
            ...
            <div tw="flex flex-col space-y-4 px-4 text-xl">
              {footerdata.map((data, index) => (
                <a href={data.link} key={index}>
                  <span tw="color[var(--greenDark)] capitalize cursor-pointer hover:color[var(--green)]">
                    {data.title}
                  </span>
                </a>
              ))}
